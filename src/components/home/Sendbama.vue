<template>
  <div class="zhangbei">
    <div class="jxcontent"
         v-for="(jxitem, index) in zhangbeidata"
         :key="index">
      ...
      <ul @click="zhangbeidetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
      ...
      <div class=''
           style="width:100%;height:70px;background-color:#fff; ">
...
export default {
  ...
  created: function () {
    this.axios.get('/api/zhangbeis').then((res) => {
      console.log(res)
      console.log(res.data.zhangbeis[0].data.items)
      this.zhangbeidata = res.data.zhangbeis[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
  methods: {
    ...
    zhangbeidetail (item, index) {
      // sessionStorage.setItem('itemJson', JSON.stringify(item))
      // let id = this.zhangbeidata[index].id
      // console.log(id)
