<template>
  <div class="colleagues">
    ...
    <!-- <van-tabbar>
      ...
      <van-tabbar-item icon="search">2</van-tabbar-item>
      <van-tabbar-item icon="friends-o">标签</van-tabbar-item>
      <van-tabbar-item icon="setting-o">标签</van-tabbar-item>
    </van-tabbar> -->

    <div class="jxcontent"
         v-for="(jxitem, index) in colleaguesdata"
         :key="index">
      ...
      <ul @click="colleaguesdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/colleagues').then((res) => {
      console.log(res)
      console.log(res.data.colleagues[0].data.items)
      this.colleaguesdata = res.data.colleagues[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
  methods: {
    ...
    colleaguesdetail (item, index) {
      // sessionStorage.setItem('itemJson', JSON.stringify(item))
      // let id = this.colleaguesdata[index].id
      // console.log(id)
