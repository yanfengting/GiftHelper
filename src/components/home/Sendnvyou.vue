<template>
  <div class="nvyou">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in nvyoudata"
         :key="index">
      ...
      <ul @click="nvyoudetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/nvyous').then((res) => {
      console.log(res)
      console.log(res.data.nvyous[0].data.items)
      this.nvyoudata = res.data.nvyous[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
  methods: {
    ...
    nvyoudetail (item, index) {
      // sessionStorage.setItem('itemJson', JSON.stringify(item))
      // let id = this.nvyoudata[index].id
      // console.log(id)
