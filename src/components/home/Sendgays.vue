<template>
  <div class="gays">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in gaysdata"
         :key="index">
      ...
      <ul @click="gaysdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/gays').then((res) => {
      console.log(res)
      console.log(res.data.gays[0].data.items)
      this.gaysdata = res.data.gays[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
