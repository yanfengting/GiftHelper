<template>
  <div class="baby">
    <div class="jxcontent"
         v-for="(jxitem, index) in babydata"
         :key="index">
      ...
      <ul @click="babydetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/baby').then((res) => {
      console.log(res)
      console.log('宝宝')
      this.babydata = res.data.baby[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
