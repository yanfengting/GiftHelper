<template>
  <div class="jingxuan">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in jingxuandata"
         :key="index">
      ...
      <ul @click="jingxuandetail(jxitem,index)">
        <li class="post-item">
          <!-- <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/getSwipers').then((res) => {
      console.log(res.data.swipers[0].data.banners)
      this.swiperdata = res.data.swipers[0].data.banners
    }, function (err) {
      console.log(err)
    })
