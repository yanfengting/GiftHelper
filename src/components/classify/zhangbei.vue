<template>
  <div class="zhangbeis">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in zhangbeisdata"
         :key="index">
      ...
      <ul @click="zhangbeisdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/zhangbeis').then((res) => {
      ...
      this.zhangbeisdata = res.data.zhangbeis[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
  methods: {
    ...
    zhangbeisdetail (item, index) {
      // sessionStorage.setItem('itemJson', JSON.stringify(item))
      // let id = this.zhangbeisdata[index].id
      // console.log(id)
