<template>
  <div class="wyfs">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in wyfsdata"
         :key="index">
      ...
      <ul @click="wyfsdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/wyfs').then((res) => {
      console.log(res)
      console.log(res.data.wyfs[0].data.items)
      this.wyfsdata = res.data.wyfs[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
