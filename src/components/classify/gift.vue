<template>
  <div class="gifts">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in giftsdata"
         :key="index">
      ...
      <ul @click="giftsdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/gift').then((res) => {
      console.log(res)
      console.log(res.data.gifts[0].data.categories)
      this.giftsdata = res.data.gifts[0].data.categories
    }, function (err) {
      console.log(err)
    })
  },
  methods: {
    ...
    giftsdetail (item, index) {
      // sessionStorage.setItem('itemJson', JSON.stringify(item))
      // let id = this.giftsdata[index].id
      // console.log(id)
