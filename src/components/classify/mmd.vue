<template>
  <div class="mmds">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in mmdsdata"
         :key="index">
      ...
      <ul @click="mmdsdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/mmds').then((res) => {
      console.log(res)
      console.log(res.data.mmds[0].data.items)
      this.mmdsdata = res.data.mmds[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
