<template>
  <div class="cyshs">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in cyshsdata"
         :key="index">
      ...
      <ul @click="cyshsdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/cyshs').then((res) => {
      console.log(res)
      console.log(res.data.cyshs[0].data.items)
      this.cyshsdata = res.data.cyshs[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
  methods: {
    ...
    cyshsdetail (item, index) {
      // sessionStorage.setItem('itemJson', JSON.stringify(item))
      // let id = this.cyshsdata[index].id
      // console.log(id)
