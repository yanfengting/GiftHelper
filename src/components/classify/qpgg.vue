<template>
  <div class="qpggs">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in qpggsdata"
         :key="index">
      ...
      <ul @click="qpggsdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/qpggs').then((res) => {
      console.log(res)
      console.log(res.data.qpggs[0].data.items)
      this.qpggsdata = res.data.qpggs[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
  methods: {
    ...
    qpggsdetail (item, index) {
      // sessionStorage.setItem('itemJson', JSON.stringify(item))
      // let id = this.qpggsdata[index].id
      // console.log(id)
