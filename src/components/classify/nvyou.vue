<template>
  <div class="nvyous">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in nvyousdata"
         :key="index">
      ...
      <ul @click="nvyousdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem">
...
export default {
  ...
  created: function () {
    this.axios.get('/api/nvyous').then((res) => {
      ...
      this.nvyousdata = res.data.nvyous[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
  methods: {
    ...
    nvyousdetail (item, index) {
      sessionStorage.setItem('itemJson', JSON.stringify(item))
      let id = this.nvyousdata[index].id
      console.log(id)
      this.$router.push(`/nydetail/${id}`)
    }
  }
}
</script>
