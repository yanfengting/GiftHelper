<template>
  <div class="sjgs">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in sjgsdata"
         :key="index">
      ...
      <ul @click="sjgsdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/sjgs').then((res) => {
      console.log(res)
      // console.log(res.data.sjgs[0].data.items)
      this.sjgsdata = res.data.sjgs[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
