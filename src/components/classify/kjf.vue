<template>
  <div class="kjfs">
    ...
    <div class="jxcontent"
         v-for="(jxitem, index) in kjfsdata"
         :key="index">
      ...
      <ul @click="kjfsdetail(jxitem,index)">
        <li class="post-item">
          <a class="pitem"
...
export default {
  ...
  created: function () {
    this.axios.get('/api/kjfs').then((res) => {
      console.log(res)
      console.log(res.data.kjfs[0].data.items)
      this.kjfsdata = res.data.kjfs[0].data.items
    }, function (err) {
      console.log(err)
    })
  },
